...
from battle_planner import errors
...
class Snowflake:
    ...
    def __init__(
        ...
        if cluster_id < 0 or cluster_id > SnowflakeConfig.MAX_DATACENTER_ID:
            raise errors.SystemSettingError("集群编号必须在 0-{SnowflakeConfig.MAX_DATACENTER_ID} 之间")
        if node_id < 0 or node_id > SnowflakeConfig.MAX_WORKER_ID:
            raise errors.SystemSettingError(f"节点编号必须在 0-{SnowflakeConfig.MAX_WORKER_ID} 之间")
    ...
    def generate(self) -> int:
        ...
        if timestamp < self.last_timestamp:
            raise errors.SystemSettingError(f"系统时间倒退，拒绝生成 ID 直到 {self.last_timestamp}")
